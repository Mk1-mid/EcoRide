def mostrar_top_3_mas_vendidos(inventario):
    """
    Muestra el top 3 de productos más vendidos.
    """
    top_3 = sorted(inventario, key=lambda producto: producto['ventas'], reverse=True)[:3]
    
    print("TOP 3 PRODUCTOS MÁS VENDIDOS:")
    for i, producto in enumerate(top_3, 1):
        print(f"{i}. {producto['titulo']} - {producto['ventas']} unidades vendidas")


def generar_reporte_por_autor(inventario):
    """
    Genera y muestra reporte de ventas totales agrupado por autor.
    """
    ventas_por_autor = {}
    ...
    for producto in inventario:
        ...
        if autor in ventas_por_autor:
            ventas_por_autor[autor]['cantidad'] += ventas
            ventas_por_autor[autor]['total'] += total
        else:
            ventas_por_autor[autor] = {
                'cantidad': ventas,
                'total': total
            }
    
    print("\nREPORTE DE VENTAS POR AUTOR:")
    for autor, datos in ventas_por_autor.items():
        print(f"{autor}: {datos['cantidad']} libros vendidos - ${datos['total']:.2f}")


def calcular_ingresos(inventario):
    """
    Calcula y muestra ingreso bruto y neto (con y sin descuento).
    """
    ingreso_bruto = 0
    ingreso_neto = 0
    ...
    for producto in inventario:
        ...
        ingreso_bruto += precio * ventas
        precio_con_descuento = precio * (1 - descuento/100)
        ingreso_neto += precio_con_descuento * ventas
    
    print("\nREPORTE DE INGRESOS:")
    print(f"Ingreso Bruto (sin descuentos): ${ingreso_bruto:.2f}")
    print(f"Ingreso Neto (con descuentos): ${ingreso_neto:.2f}")
    print(f"Total descuentos aplicados: ${ingreso_bruto - ingreso_neto:.2f}")estandar = 100
premiun = 100
